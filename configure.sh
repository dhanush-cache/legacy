...
install_ext() {
    install "Wget" "jq" "gunzip" "wget" "gunzip" "jq"
    extentions=("$@")
    for editor in code code-server; do
        for extension in "${extentions[@]}"; do
            if command -v "$editor" >/dev/null && ! "$editor" --list-extensions | grep -iq "$extension"; then
                pub=${extension%.*}
                ext=${extension#*.}
                loading "Installing ${c[33]}${ext}${c[32]} by ${c[34]}${pub}${c[32]}..." &
                "$editor" --install-extension "${extension}" >/dev/null 2>&1
                if ! "$editor" --list-extensions | grep -iq "$extension"; then
